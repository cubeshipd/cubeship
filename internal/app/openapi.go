	...
	return openapi.Spec{
		...
		Schemas: mergeSchemas(mergeSchemas(project.EnvSchemas(), metrics.Schemas()), map[string]*openapi.Schema{
			"Deployment": openapi.Object(map[string]*openapi.Schema{
				...
				"deletable":  openapi.Bool("Whether this record may be removed. False only for a deploy still running, which the daemon is still writing to."),
