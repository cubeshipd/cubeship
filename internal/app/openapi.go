	...
	return openapi.Spec{
		...
		Schemas: mergeSchemas(project.EnvSchemas(), map[string]*openapi.Schema{
			...
			"App": openapi.Object(map[string]*openapi.Schema{
				...
				"source":      {Type: "string", Enum: []string{"registry", "external"}, Description: "Where this app's image comes from. \"registry\" is an image pushed to Cubeship, and the push is what deploys it; \"external\" is an image in a registry Cubeship does not run, which nothing notifies it about, so those deploy only when asked."},
				"org":         openapi.String(""),
			...
			"/apps": {
				"post": {
					...
					RequestBody: openapi.Body(openapi.Object(map[string]*openapi.Schema{
						...
						"source":      {Type: "string", Enum: []string{"registry", "external"}, Description: `Where the image comes from. "registry" (the default) is an image you push to Cubeship, and the push deploys it. "external" is an image in a registry Cubeship does not run.`},
