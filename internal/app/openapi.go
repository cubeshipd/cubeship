	...
	return openapi.Spec{
		...
		Schemas: mergeSchemas(mergeSchemas(project.EnvSchemas(), metrics.Schemas()), map[string]*openapi.Schema{
			"Deployment": openapi.Object(map[string]*openapi.Schema{
				...
				"deletable":  openapi.Bool("Whether this record may be removed. False for one still running, and for the one the app is running."),
		...
		Paths: map[string]openapi.PathItem{
			...
			appPath + "/deployments/{id}": {
				...
				"delete": {
					...
					Description: "**It removes a record, not a container.** Nothing here stops an app: the container belongs to the app and this row is the history of how it got there. What goes with the row is the build log, which is most of its bytes. The image stays in the registry, which needs a garbage collection pass Cubeship does not run — the same thing that is true when an app itself is deleted.\n\nTwo are refused, and `deletable` on the deployment says so in advance. One that is **still running**, because the deploy is writing to that row. And the one the app **is running**, because its record is the only thing that says what the container is — losing it leaves something running that nothing on this instance explains.\n\nEverything else is history, and a failed deploy is history nobody wants to keep looking at. The role is the one that deploys this app: somebody who may add to the history may tidy it.",
					Tags:        []string{"Apps"},
					Parameters: append(refParams,
						openapi.PathParam("id", "The deployment's id.")),
					Responses: openapi.Responses{
						"204": openapi.Empty("The record is gone. The app is untouched."),
						"400": openapi.TextResponse("The id is not a number."),
						"401": openapi.Unauthorized,
						"403": openapi.Forbidden,
						"404": openapi.NotFound,
						"409": openapi.TextResponse("The deploy is still running, or it is the one the app is running."),
					},
				},
			},
			appPath + "/env": {
