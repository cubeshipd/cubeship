	...
	return openapi.Spec{
		...
		Schemas: mergeSchemas(project.EnvSchemas(), map[string]*openapi.Schema{
			...
			"App": openapi.Object(map[string]*openapi.Schema{
				...
				"source":      {Type: "string", Enum: []string{"registry", "external", "dockerfile"}, Description: "Where this app's image comes from. \"registry\" is an image pushed to Cubeship, and the push is what deploys it; \"external\" is an image in a registry Cubeship does not run; \"dockerfile\" is built here from a Git repository. Only a push to Cubeship's own registry deploys on its own — the other two deploy when asked."},
				"repo":        openapi.String("For a dockerfile app, the Git repository it builds from."),
				"ref":         openapi.String("For a dockerfile app, the branch, tag or commit built. Empty means the repository's default branch."),
			...
			"/apps": {
				"post": {
					...
					RequestBody: openapi.Body(openapi.Object(map[string]*openapi.Schema{
						...
						"source":      {Type: "string", Enum: []string{"registry", "external", "dockerfile"}, Description: `Where the image comes from. "registry" (the default) is an image you push to Cubeship, and the push deploys it. "external" is an image in a registry Cubeship does not run. "dockerfile" is built here from a Git repository, and needs the admin role — a build runs whatever the repository contains, on this host.`},
						...
						"repo":        openapi.String(`Required for a dockerfile app, and refused for any other: the Git repository to build. An https://, http:// or git:// URL — ssh needs a key this instance does not have. Only https authenticates what comes back, and a build runs whatever comes back, so use it for anything reachable from the internet. Do not put a "#ref" on it; the ref is its own field.`),
						"ref":         openapi.String(`For a dockerfile app: the branch, tag or commit to build. Defaults to the repository's default branch, and a deploy can name a different one.`),
						"dockerfile":  openapi.String(`For a dockerfile app: the recipe's path within the repository. Defaults to "Dockerfile" at the root.`),
