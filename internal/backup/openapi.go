	...
	return openapi.Spec{
		...
		Schemas: map[string]*openapi.Schema{
			"Backup": openapi.Object(map[string]*openapi.Schema{
				...
				"off_machine":     openapi.Bool("Whether it is somewhere other than the disk it was taken from. **False is not a backup** in the sense that matters — it survives somebody dropping a table and not the machine — and every screen showing one says so.\n\nIt is **not** \"went to an object store\": a *managed* store is a MinIO container this instance runs, with its objects in a bind mount under the data directory, so a dump sent there is on the same disk as the database. Recorded when the dump is taken rather than worked out on read, because the store it went to can be deleted — which is exactly when somebody needs to know what they still have."),
				...
				"status":          {Type: "string", Enum: []string{"taking", "succeeded", "failed"}, Description: "A row is written before the dump starts, for the reason a deployment row is: the work is detached and where the outcome goes has to exist before there is one."},
			...
			"BackupSchedule": openapi.Object(map[string]*openapi.Schema{
				...
				"store":       openapi.String("The object store they go to, by name. Absent for this machine's own disk."),
				"bucket":      openapi.String("The bucket in that store."),
			...
			"/datastores/{name}/backups/schedule": {
				...
				"put": {
					...
					RequestBody: openapi.Body(openapi.Object(map[string]*openapi.Schema{
						...
						"store":    openapi.String("Which object store to put them in, by name. Leave it out for this machine's own disk, which works with nothing configured and is **not a backup**: same disk, same machine, same fire."),
						"bucket":   openapi.String("The bucket in that store. Required when one is named."),
					}, "at")),
					Responses: openapi.Responses{
						"200": openapi.JSONResponse("The schedule.", openapi.Ref("BackupSchedule")),
