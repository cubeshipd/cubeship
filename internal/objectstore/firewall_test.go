...
// firewallFakeDocker is enough of DockerAPI to let a managed store
...
// no app module wired in: Expose, Unexpose, Delete and Link never reach
// it, and every test here is about the firewall rule those go through
// instead.
func newFirewallFixture(t *testing.T, host firewall.Host) *Service {
	...
	svc := NewService(db, credential.NewService(db), nil, prov,
		settings.NewService(db), metrics.NewService(db), host)
	t.Cleanup(svc.WaitForProvisioning)
	return svc
}

func createManaged(t *testing.T, svc *Service, slug string) *Store {
	t.Helper()
	store, err := svc.Create(context.Background(), firewallTestAdmin, ManagedSpec{Slug: slug})
	if err != nil {
...
// published port never matches what arrives addressed to 9000.
// Exposing has to write MinIO's own port, never the one handed back to
// whoever asked.
func TestExposeAdmitsTheInsidePortNotThePublishedOne(t *testing.T) {
	...
	store := createManaged(t, svc, "media")

	if _, err := svc.Expose(context.Background(), firewallTestAdmin, store.Slug, 0); err != nil {
		t.Fatalf("expose: %v", err)
	}
	svc.WaitForProvisioning()

	want := "ufw route allow proto tcp from any to any port 9000 comment cubeship"
	var wrote bool
	for _, cmd := range host.ran {
		// Every port this range hands out starts with "16", so a rule
		// naming the published one instead of Port would show up as
		// exactly this substring.
		if strings.Contains(cmd, "port 16") {
...
// Create never goes through setPort, so a store exposed the moment it
// is made — ManagedSpec.Expose, not a later call to Expose — needs its
// own admit or it would sit unreachable until the daemon's next start
// swept it up.
func TestCreatingAnAlreadyExposedStoreAdmitsItsPort(t *testing.T) {
	host := &firewallFakeHost{available: true}
	svc := newFirewallFixture(t, host)
	port := PortRangeStart
	_, err := svc.Create(context.Background(), firewallTestAdmin, ManagedSpec{
		Slug: "media", Expose: &port,
	})
	if err != nil {
	...
	t.Errorf("no rule was admitted for a store exposed at creation: %v", host.ran)
}

// Every managed store listens on the same Port, so unexposing one of two
// must not close the rule the other still needs.
func TestUnexposingOneOfTwoManagedStoresDoesNotRemoveTheRule(t *testing.T) {
	host := &firewallFakeHost{available: true}
	svc := newFirewallFixture(t, host)
	media := createManaged(t, svc, "media")
	backups := createManaged(t, svc, "backups")

	if _, err := svc.Expose(context.Background(), firewallTestAdmin, media.Slug, 0); err != nil {
		t.Fatalf("expose media: %v", err)
	}
	if _, err := svc.Expose(context.Background(), firewallTestAdmin, backups.Slug, 0); err != nil {
		t.Fatalf("expose backups: %v", err)
	}
	svc.WaitForProvisioning()

	if _, err := svc.Unexpose(context.Background(), firewallTestAdmin, media.Slug); err != nil {
		t.Fatalf("unexpose media: %v", err)
	}
	svc.WaitForProvisioning()

	for _, cmd := range host.ran {
		if strings.Contains(cmd, "delete") && strings.Contains(cmd, "port 9000") {
			t.Fatalf("the shared rule was removed while backups still needs it: %v", host.ran)
		}
	}
}

// And once the last managed store on the port is unexposed, the rule
// goes.
func TestUnexposingTheLastManagedStoreRemovesTheRule(t *testing.T) {
	host := &firewallFakeHost{available: true}
	svc := newFirewallFixture(t, host)
	store := createManaged(t, svc, "media")
	...
	if _, err := svc.Unexpose(context.Background(), firewallTestAdmin, store.Slug); err != nil {
		t.Fatalf("unexpose: %v", err)
	}
	svc.WaitForProvisioning()

	want := "ufw route delete allow proto tcp from any to any port 9000 comment cubeship"
	for _, cmd := range host.ran {
		if cmd == want {
			return
		}
...
// A linked store has no container and no inside port to admit or
// withdraw — it never reaches the firewall at all, in either direction.
func TestALinkedStoreWritesNoFirewallRule(t *testing.T) {
	host := &firewallFakeHost{available: true}
	svc := newFirewallFixture(t, host)

	store, err := svc.Link(context.Background(), firewallTestAdmin, LinkSpec{
	...
	if _, err := svc.Delete(context.Background(), firewallTestAdmin, store.Slug); err != nil {
		t.Fatalf("delete: %v", err)
	}
	if len(host.ran) != 0 {
		t.Errorf("a linked store touched the firewall: %v", host.ran)
	}
}

// AdmitExposed is what repairs an instance exposed before this existed:
// every already-exposed managed store gets its rule the next time the
// daemon starts.
func TestAdmitExposedWritesTheRuleForWhatIsAlreadyExposed(t *testing.T) {
	...
	t.Errorf("the sweep did not admit an exposed store: %v", host.ran)
}

// A daemon with no way onto the host does not stop to list every store
// first — there would be nothing to do with the answer.
