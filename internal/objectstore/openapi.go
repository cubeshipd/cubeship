	...
	return openapi.Spec{
		...
		Schemas: withMetrics(map[string]*openapi.Schema{
			...
			"ObjectStoreProvider": openapi.Object(map[string]*openapi.Schema{
				...
				"scopes_by_bucket": openapi.Bool("Whether this provider's own logins are commonly issued for a single bucket — R2's tokens, a Space's access keys. Every provider accepts the optional `bucket` field; this says how a form should phrase it. Where it is true, naming one is the expected answer; where it is false a login usually reaches the whole account, so pinning is the unusual choice and worth a word about what it gives up."),
			...
			"/objectstores": {
				...
				"post": {
					...
					RequestBody: openapi.Body(openapi.Object(map[string]*openapi.Schema{
						...
						"bucket":   openapi.String("external: pin this store to one bucket, for a login that was issued for exactly one and cannot list them — an R2 token, a Space's access key, an IAM policy narrowed to one. Omit for the normal case, which is a login that reaches the account.\n\nAccepted on every provider, because a credential scoped to one bucket is a thing somebody can hold anywhere. What it costs is listing, creating and deleting every other bucket in the store — undone from the store's own settings by sending an empty `bucket`, never by re-linking."),
			...
			"/objectstores/{name}": {
				...
				"patch": {
					...
					RequestBody: openapi.Body(openapi.Object(map[string]*openapi.Schema{
						...
						"bucket":        openapi.String("The one bucket this store is pinned to. **Empty unpins it**, which is a value rather than a gap — leaving the field out entirely is the only way of saying \"as it is\", so saving a description cannot unpin a store by omission.\n\nSetting one is refused on a managed store, which lists its own buckets, and where an attached app names a different bucket — the store would claim to be one bucket while an attachment said otherwise, and the screen would stop being able to show the bucket that app is using."),
