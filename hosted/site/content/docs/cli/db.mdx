...
| `--extension` |  | a Postgres extension to create the database with — pgvector, vectorchord. Repeat for more than one. `db extension add` installs more later; nothing removes one. See `db engines` |
...
## cubeship db extension

Install Postgres extensions on a database that already exists.
...
## cubeship db extension add

Install extensions on a database.
...
```bash
cubeship db extension add <name> [flags]
```

| Flag | Default | What it does |
| --- | --- | --- |
| `--extension` |  | an extension to install — pgvector, vectorchord. Repeat for more than one |
