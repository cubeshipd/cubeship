...
| `--tag` |  | image tag to deploy (default: the tag the app is on) |
...
Open an interactive shell inside an app's running container.
...
```bash
cubeship app shell <app> [flags]
```

| Flag | Default | What it does |
| --- | --- | --- |
| `--server` |  | the server whose copy to open it in (default: the first running copy) |

## cubeship app tcp

Publish a port of an app's container on a host port of the control plane,
...
## cubeship app tcp add

Publish \<container-port\> on a host port of the control plane, from the
...
```bash
cubeship app tcp add <app> <container-port> [flags]
```

| Flag | Default | What it does |
| --- | --- | --- |
| `--host-port` |  | the host port to publish on; picked from 17000-17999 when left out |

## cubeship app tcp list

List an app's published TCP ports

```bash
cubeship app tcp list <app>
```

## cubeship app tcp remove

Stop publishing a port. The container running now keeps it until the
next deploy.

```bash
cubeship app tcp remove <app> <id>
```

## cubeship app volume

A volume is a directory mounted into an app's container whose contents
...
## cubeship app volume add

Give an app a volume at \<path\>, an absolute path inside its container.

It is mounted from the next deploy. Refused while the app runs as more
than one copy, on more than one server, or autoscales.

```bash
cubeship app volume add <app> <path>
```

## cubeship app volume backup

Back up and restore one of an app's volumes. The app is stopped while a
copy is taken or put back, and started again afterwards. Requires the
admin role.

## cubeship app volume backup list

List a volume's backups, newest first

```bash
cubeship app volume backup list <app> <volume-id>
```

## cubeship app volume backup restore

Replace a volume's data with what it held when the backup was taken.
This cannot be undone. The app is stopped while it happens.

```bash
cubeship app volume backup restore <backup-id> [flags]
```

| Flag | Default | What it does |
| --- | --- | --- |
| `--server` |  | restore on this server instead, which moves the app there; the copy on the old server is left |
| `--yes` |  | confirm the restore |

## cubeship app volume backup take

Back a volume up now, stopping the app for the copy

```bash
cubeship app volume backup take <app> <volume-id> [flags]
```

| Flag | Default | What it does |
| --- | --- | --- |
| `--bucket` |  | the bucket in that store |
| `--store` |  | the S3 store linked from outside this instance to send it to; defaults to the volume's schedule's |

## cubeship app volume list

List an app's volumes

```bash
cubeship app volume list <app>
```

## cubeship app volume remove

Take a volume off an app. Its data is kept on the server unless
--delete-data is given, which cannot be undone. The container running
now keeps the directory until the next deploy.

```bash
cubeship app volume remove <app> <id> [flags]
```

| Flag | Default | What it does |
| --- | --- | --- |
| `--delete-data` |  | delete the volume's data too, which cannot be undone |
