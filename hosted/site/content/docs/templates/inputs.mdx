...
On an instance's dashboard the installer picks the name the way they add
one to an app: a connected DNS provider, a zone and a subdomain — and the
`A` record pointing at the instance is written before the install starts —
or a name under the instance's own domain, or one typed by hand for DNS
kept elsewhere.
