	...
	return openapi.Spec{
		...
		Schemas: withMetrics(map[string]*openapi.Schema{
			...
			"Datastore": openapi.Object(map[string]*openapi.Schema{
				...
				"backup_consistency": openapi.String("What a dump of this engine actually promises, in a sentence — it differs per engine in a way no general wording covers, and a standalone MongoDB cannot give a consistent snapshot across collections at all. Absent for an engine that is not backed up."),
