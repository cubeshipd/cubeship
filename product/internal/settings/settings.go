...
const (
	...
	// BetaVersions is "true" when the update check offers beta versions —
	// every prerelease, betas and release candidates alike — as well as
	// stable releases. Empty is stable only.
	//
	// It changes what is offered and nothing else: automatic updating
	// stays on stable releases whatever this says, because a beta is
	// something somebody chooses to try, not something that should arrive
	// on its own at three in the morning.
	BetaVersions = "beta_versions"
