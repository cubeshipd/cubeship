...
var toolRules = map[string]toolRule{
	...
	"list_app_domains":    tool(user.ResApps, user.LevelView),
	"add_app_domain":      tool(user.ResDomains, user.LevelManage),
	"set_app_domain_port": tool(user.ResDomains, user.LevelManage),
	"remove_app_domain":   tool(user.ResDomains, user.LevelManage),
	...
	"add_datastore_extensions": tool(user.ResDatabases, user.LevelManage),
	"list_datastores":          own,
