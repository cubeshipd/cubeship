...
The project, app, database and object-store cards end in two bars, CPU
...
project has no limit of its own; an app's replicas are added up too. **A
bar is a share, so it needs a ceiling**, and the one-core convention has
none: CPU is drawn against the container's own limit, or against every
core of the machine when it has no limit, and memory against the
cgroup's ceiling. Anything with no reading — stopped, not deployed, or a
linked store with nothing here to read — keeps its bars, at 0.
