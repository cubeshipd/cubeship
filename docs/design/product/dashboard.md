...
`product/dashboard/` is a Next.js app with `output: "standalone"`, built by
its own `Dockerfile` into its own image — `cubeship`, beside the daemon's
...
`internal/dashboard` is the proxy, and the one thing it does beyond proxying
...
`make dashboard-dev` runs the dashboard on :3001 with hot reload, and a daemon
...
**A picture, a name, a ring, and what its apps use.** The usage is a
footer of two bars, CPU and memory, adding up every app in the project
as a share of the machine — see [monitoring.md](monitoring.md). It used to carry three more things
...
The same shape as a project's — `ResourceCard`, CPU and memory bars
...
commands. Both are vendored as woff2 under `product/dashboard/src/fonts` and loaded
